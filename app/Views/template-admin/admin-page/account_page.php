...
<?php endif; ?>
<div class="flex flex-col md:flex-row space-x-0 space-y-2 md:space-y-0 md:space-x-6">
    <div class="flex flex-col justify-between tetx-base md:text-xl font-semibold text-slate-800 bg-white px-4 md:px-10 py-4 md:py-6 w-full rounded-md shadow-md  shadow-[#1F3C88]/25">
        <h1 class="border-b pb-2">Informasi tentang akun</h1>
        <div class="flex flex-col justify-center items-start space-y-1 py-4">
            <p class="font-medium text-base md:text-lg">Nama    : <span><?= $userName?></span></p>
            <p class="font-medium text-base md:text-lg">Email   : <span><?= $userEmail ?></span></p>
        </div>
        <a class="flex w-full lg:w-1/3 justify-end" href="<?= base_url('/admin-page-logout-process') ?>">
    ...
    <div class="text-base md:text-xl font-semibold text-slate-800 bg-white px-4 md:px-10 py-4 md:py-6 w-full rounded-md shadow-md shadow-[#1F3C88]/25">
        <h1>Ubah password</>
        <form action="<?= base_url('admin/account/change-password') ?>" method="POST" class="flex flex-col justify-center items-start space-y-2 mt-2 border-t py-4 font-medium text-sm">
            <div class="flex justify-between items-center border border-gray-300 space-x-2 rounded-md w-full">
                <input minlength="8" id="new_password" class="px-4 py-2 w-full focus:outline-none focus:ring-0 focus:border-[#1F3C88]" type="password" name="new_password" placeholder="Password baru (8 karakter) *">
            ...
            <div class="flex justify-between items-center border border-gray-300 space-x-2 rounded-md w-full">
                <input minlength="8" id="confirm_password" class="px-4 py-2 w-full focus:outline-none focus:ring-0 focus:border-[#1F3C88]" type="password" name="confirm_password" placeholder="Konfirmasi password baru (8 karakter) *">
